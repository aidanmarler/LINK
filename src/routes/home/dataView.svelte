...
{#if options.showKey}
	<div class="flex flex-wrap justify-center {options.large == true ? 'text-md' : 'text-xs'} font-normal">
		{#if completionReport.complete > 0}
			<div class="justify-center mx-0.5 flex flex-row p-1 rounded-md">
				<div class="bg-green-700 dark:bg-green-500 rounded-md opacity-90 {options.large ? ' mt-1 h-3 w-3 ' : ' h-2 w-2 '}"></div>
				<p class="-mt-1 px-1">{completionReport.complete} Complete</p>
			</div>
		{/if}
		{#if completionReport.needsReview > 0}
			<div class="justify-left mx-0.5 flex flex-row p-1 rounded-full">
				<div class="bg-amber-600 dark:bg-amber-700 rounded-full opacity-90 {options.large ? ' mt-1 h-3 w-3 ' : ' h-2 w-2 '}"></div>
				<p class="-mt-1 px-2">{completionReport.needsReview} To Review</p>
			</div>
		{/if}
		{#if completionReport.incomplete > 0}
			<div class="justify-left mx-0.5 flex flex-row p-1 rounded-full">
				<div class="bg-pink-900/40 dark:bg-pink-700/40 rounded-full opacity-90 {options.large ? ' mt-1 h-3 w-3 ' : ' h-2 w-2 '}"></div>
