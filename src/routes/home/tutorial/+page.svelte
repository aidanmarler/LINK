<script lang="ts">
	...
	const formattedLang = $derived(capitalizeFirstLetter(profile.language ? profile.language : ''));
</script>
...
>
	...
	<div class=" text-xl border-b-2 border-dashed border-inherit pb-4 px-4">
		<ol class="p-2">
			<li class="mb-1">
				<span class="font-bold">Choose a CRF to translate</span> by clicking "CRF to Translate" and then
				selecting the CRF you would like to work on.
			</li>
			<li>
				<span class="font-bold">Find a segment</span> by either using "Go to Next Segment" or navigating
				manually from either "ARC Questions" or "Listed Options"
			</li>
		</ol>
	</div>
	<div class="w-full flex justify-center">
	...
	<div class=" text-xl border-dashed border-inherit pb-2 px-4">
		...
		<ol class="p-2">
			<li class="mb-1">
				<span class="font-bold">Step 1: Translation -></span> Write out the provided English segment
				in {formattedLang}.
			</li>
			<li>
	...
	<div class="border-b-2 border-dashed border-inherit p-4">
		...
		<p class="px-2 pb-2">
			...
			> for when you have something to say about a translation, review, or the segment. These will be
			visible for all other reviewers.
		</p>
