<script lang="ts">
	...
	onMount(async () => {
		...
		console.log("data.pathSegments: ",data.pathSegments)
		//const loadedData = await data.dataPromise;
		//const segs = initializeTraversal(loadedData.locationTree, data.pathSegments, true);
		//if (!segs) return;
		//const nextSegment = segs[0];
		//const lastSegment = initializeTraversal(loadedData.locationTree, data.pathSegments, false);
	...
	async function onsubmit(shouldContinue: boolean, forward: boolean) {
		...
		//const nextSegment = initializeTraversal(resolvedData.locationTree, data.pathSegments, forward);
		...
		const nextSlug = getSegmentSlug(nextSegment?.segmentIds[0], resolvedData.locationTree, '/home');
